...
![Gemini_Generated_Image_m79icum79icum79i (1).png](Delegating%20Computations%20of%20Multi-Scalar%20Multiplica/Gemini_Generated_Image_m79icum79icum79i_(1).png)
...
![Gemini_Generated_Image_6r5u3f6r5u3f6r5u.png](Delegating%20Computations%20of%20Multi-Scalar%20Multiplica/Gemini_Generated_Image_6r5u3f6r5u3f6r5u.png)
...
1. **Split the Data:** The protocol takes vectors of length $m$ and splits them into **Left (**$L$**)** and **Right (**$R$**)** halves of size $m/2$, say $\mathbf{P} = (\mathbf{P}_L, \mathbf{P}_R)$ and $\mathbf{d} = (\mathbf{d}_L, \mathbf{d}_R)$;
2. **Send Cross-Terms:** The prover calculates how these two halves interact and sends these results (the so-called **cross-terms**) to the verifier in form of a commitment; this allows the verifier to keep track of the math as the data is compressed;
3. **The Verifier’s Challenge:** To ensure the prover isn't cheating, the verifier sends a random challenge $x$;
4. **Fold the Statement:** Using the challenge $x$, both parties squash the two halves into a single new vectors that is half the original size:
    
    $$
    \mathbf{P}' = \mathbf{P}_L \oplus \left(\mathbf{P}_R \bullet x\right), \quad \mathbf{d}' = \mathbf{d}_L + x^{-1}\cdot \mathbf{d}_R.
    $$
    
5. **Update the Inner Product:** The new inner product $U'=\mathbf{P}' \bullet \mathbf{d}'$  is computed and used to call the recursive step.
6. **Final Base Case:** In the final step, the verifier performs a single scalar multiplication to check the final, reduced statement.
...
![Gemini_Generated_Image_evdhdrevdhdrevdh.png](Delegating%20Computations%20of%20Multi-Scalar%20Multiplica/Gemini_Generated_Image_evdhdrevdhdrevdh.png)
...
1. **Challenge:** The verifier picks $m$ random coefficients $r_0, \dots, r_{m-1}$.
2. **Left Side Check:** The verifier computes a *small* MSM of the $w_j$ values: 
    
    $$
    W' = \bigoplus_{j=0}^{m-1} r_j w_j.
    $$
    
    This is fast because $m$ *is small* (e.g., $256$) *compared to $N$*.
    
3. **Right Side Check:** The verifier computes an $N$-wide MSM:
...
4. **Equality:** If $W' = W''$, the $w_j$ values are correct with high probability.
5. **Final Result:** Once verified, the verifier computes the final $Q$ from $w_j$ using simple doubling and addition:
...
![Gemini_Generated_Image_fzwibfzwibfzwibf.png](Delegating%20Computations%20of%20Multi-Scalar%20Multiplica/Gemini_Generated_Image_fzwibfzwibfzwibf.png)
...
1. **The Output Claim:** The Prover claims to the Verifier that *the result of the computation is* $Y$.
2. **Polynomial Representation:** The Verifier treats the values of the gates in each layer as a function. Specifically, they use a **Multilinear Extension (MLE)**. This turns gate values into a polynomial.
...
3. **Layer Reduction:** To verify the claim about Layer $i$, the Prover and Verifier run a Sum-Check protocol;
...
4. **The Chain Reaction:** This process repeats layer by layer.
5. **The Final Verification:** Once they reach the final layer (the Input Layer), the Verifier simply checks the remaining claim against the original input data (or a cryptographic commitment to it). If it matches, the entire computation (of $Y$) is proven correct.
...
![Gemini_Generated_Image_3c8c4y3c8c4y3c8c.png](Delegating%20Computations%20of%20Multi-Scalar%20Multiplica/Gemini_Generated_Image_3c8c4y3c8c4y3c8c.png)
...
![Gemini_Generated_Image_y3sr3ty3sr3ty3sr.png](Delegating%20Computations%20of%20Multi-Scalar%20Multiplica/Gemini_Generated_Image_y3sr3ty3sr3ty3sr.png)
...
![Gemini_Generated_Image_m79icum79icum79i.png](Delegating%20Computations%20of%20Multi-Scalar%20Multiplica/Gemini_Generated_Image_m79icum79icum79i.png)
